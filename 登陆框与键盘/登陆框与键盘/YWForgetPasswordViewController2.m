//
//  YWForgetPasswordViewController2.m
//  YunWan
//
//  Created by luoyin on 15/3/23.
...
@interface YWForgetPasswordViewController2 ()<UITextFieldDelegate>
...
@implementation YWForgetPasswordViewController2{
...
#define kQQLoginViewHeight                 32.0
    ...
    self.resetPasswordButton = ({
        ...
        [button setTitle:@"正在重置密码..." forState:UIControlStateDisabled];
        [button setTitleColor:YunWanButtonDisableTitleColor
                     forState:UIControlStateDisabled];
        button.titleLabel.font = fontForButton;
        [button setTitleColor:textColorForButton forState:UIControlStateNormal];
        button.backgroundColor = YunWanButtonBackgroundRedColor;
        [button addTarget:self
                   action:@selector(resetPasswordNow:)
         forControlEvents:UIControlEventTouchDown];
        button;
    });
    [self.registerView addSubview:self.resetPasswordButton];
    ...
    [self.secondPasswordTextField makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.resetPasswordButton.top).offset(-kVSpacingBetweenTextFieldAndButton);
    }];
    ...
    [self.resetPasswordButton makeConstraints:^(MASConstraintMaker *make) {
        UIView *superView = self.registerView;
        make.left.equalTo(superView).insets(registerViewInsets);
        make.right.equalTo(superView).insets(registerViewInsets);
        make.height.equalTo(@(kTextFieldHeight));
...
- (void)viewDidLoad {
    ...
    [UIColor colorWithRed:71/255.0 green:161/255.0 blue:213/255.0 alpha:1.0];
...
// 重置密码
- (void)resetPasswordNow:(id)sender {
    // 重置密码
}

//// 登录成功
//- (void)didResetPassWordSucceed:(NSDictionary *)dict {
//    DLog(@"重置密码成功");
//    
//    if (self.loginState == LoginStatePush) {
//        
//        AppDelegate *appDelegte = [UIApplication sharedApplication].delegate;
//        appDelegte.userInfo = [[UserInfo alloc] initWithDic:dict];
//        
//        MainNavigationController *mainNavigationController =
//        (MainNavigationController *)self.navigationController;
//        
//        [mainNavigationController showSideMenuVC];
//        
//    } else if (self.loginState == LoginStateModally) {
//        AppDelegate *appDelegte = [UIApplication sharedApplication].delegate;
//        appDelegte.userInfo = [[UserInfo alloc] initWithDic:dict];
//        
...
// 验证手机号
- (BOOL)validPhoneNumberWithString:(NSString *)string {
    NSError *error;
    BOOL b = FALSE;
...
// 验证验证码
- (BOOL)vertifyVertifyCodeWithString:(NSString *)string {
    
    NSError *error;
    BOOL vertifyCodeOK = [NSString isValidVerifyCode:string error:&error];
    ...
    return vertifyCodeOK;
}

// 验证第一个密码
- (BOOL)vertifyFirstPasswordWithString:(NSString *)string {
    
    NSError *error;
    BOOL firstPasswordOK = [NSString isValidPassword:string error:&error];
    // TODO 密码验证需要重新弄一下
    if (firstPasswordOK) {
        self.firstPasswordImageView.image = [UIImage imageNamed:VERTIRY_OK_IMAGE_NAME];
    } else {
    ...
    return firstPasswordOK;
}

// 验证第二个密码
- (BOOL)vertifySecondPasswordWithString:(NSString *)string {
    
    NSError *error;
    BOOL b1 = [NSString isValidPassword:string error:&error];
    if (!b1) {
        self.secondPasswordImageView.image = [UIImage imageNamed:VERTIRY_ERROR_IMAGE_NAME];
        return b1;
    }
    BOOL secondPasswordOK =
    [self.firstPasswordTextField.text isEqualToString:self.secondPasswordTextField.text];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    textField.layer.borderColor = YunWanNavigationBackgroundLightBlueColor.CGColor;
    
    if (textField == self.userNameTextField) {
        [self validPhoneNumberWithString:self.userNameTextField.text];
    } else if (textField == self.vertifyCodeTextField) {
        [self vertifyVertifyCodeWithString:self.vertifyCodeTextField.text];
    } else if (textField == self.firstPasswordTextField) {
        [self vertifyFirstPasswordWithString:self.firstPasswordTextField.text];
    } else if (textField == self.secondPasswordTextField) {
        [self vertifySecondPasswordWithString:self.secondPasswordTextField.text];
    }
    return true;
}
